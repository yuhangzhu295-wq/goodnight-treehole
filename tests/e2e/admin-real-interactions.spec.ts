...
describe('admin real interaction manifest', () => {
  it('keeps the admin click-all manifest wired to the Playwright runner', () => {
    ...
    expect(manifest.some((item) => item.selector === 'admin-post-approve' && item.expectedApi === 'PATCH /api/admin/v1/posts/:id/moderation')).toBe(true);
    expect(manifest.some((item) => item.selector === 'admin-route-test' && item.expectedApi === 'POST /api/admin/v1/ai/routes/:style/test')).toBe(true);
  });
});
