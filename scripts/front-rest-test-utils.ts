...
import { ensureFrontRestDirs, inspectElementFromPoint, killPorts, waitForUrl } from './diagnose/front-rest-common';
...
export async function startFrontRestStack(name: string) {
  ...
  const databaseName = `goodnight_treehole_test_${name.replace(/[^a-z0-9]+/gi, '_').toLowerCase()}`;
  const env = {
    DATABASE_URL: resetTestDatabase(databaseName),
    GOODNIGHT_STORE_FILE: `data/goodnight-store.${name}.json`,
    VITE_API_BASE_URL: restUrls.api,
  };
  const procs = [
  ...
  await waitForUrl(`${restUrls.api}/api/v1/posts`);
  await waitForUrl(`${restUrls.front}/pages/tool/index`);
  return procs;
}
...
export async function screenshot(page: Page, phase: 'before' | 'after', name: string) {
  const path = `artifacts/screenshots/front-rest/${phase}/${name}.png`;
