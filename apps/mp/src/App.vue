...
const activeTab = computed(() => {
  if (route.path.startsWith('/pages/reply') || route.path.startsWith('/pages/letter')) return 'letter';
  if (route.path.startsWith('/pages/tool')) return 'tool';
...
<template>
  <main class="phone-shell">
    ...
    <nav v-if="showTabbar" class="tabbar" aria-label="底部导航">
      <RouterLink data-testid="tab-square" to="/pages/square/index" :class="{ 'router-link-active': activeTab === 'square' }">
      ...
      <RouterLink data-testid="tab-letter" to="/pages/letter/index" :class="{ 'router-link-active': activeTab === 'letter' }">
        <span class="tab-icon" aria-hidden="true">
          <svg viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14" rx="2" /><path d="m4 7 8 6 8-6" /></svg>
        </span>
        <span>回信</span>
      </RouterLink>
      <RouterLink data-testid="tab-tool" to="/pages/tool/index" :class="{ 'router-link-active': activeTab === 'tool' }">
        <span class="tab-icon" aria-hidden="true">
