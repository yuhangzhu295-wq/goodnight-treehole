...
export class FollowUpWorkerService implements OnModuleInit, OnModuleDestroy {
  ...
  private async deliver(input: { id: string; kind: string; userId: string; journeyId?: string; payload?: Record<string, unknown> }) {
    ...
    const existingNotification = await this.prisma.userNotification.findUnique({ where: { id: notificationId } });
    if (!existingNotification) {
      const message = this.notificationCopy(input.kind);
    ...
    return { notificationId, status: 'delivered' };
  }

  private notificationCopy(kind: string) {
    if (kind === 'FUTURE_SELF') return { type: 'FUTURE_SELF', title: '清醒时候的你，留了一句话', body: '这是过去的你留给现在的。', targetRoute: '/pages/future-self/index' };
    if (kind === 'DECISION_COOLDOWN') return { type: 'COOLDOWN_RELEASED', title: '现在还想这样做吗？', body: '你之前放进决定保险箱的事情，已经到了可以重新看一眼的时间。', targetRoute: '/pages/action/index?section=vault' };
